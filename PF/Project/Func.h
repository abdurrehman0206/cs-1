...
void ct_input(string co_te_ch[][3], int *c_num)
{
    cin.ignore();
    for (int i = 0; i < *c_num; i++)
    {
        ...
        cout << "Enter Course Credits for " << co_te_ch[i][0] << " :: ";
        getline(cin, co_te_ch[i][2]);
        cout << endl;
    }
    system("pause");
    system("CLS");
...
void ct_output(string co_te_ch[][3], int *c_num)
{
    cout << "Course \t Teacher \t Crd.Hrs" << endl;
    cout << endl;
    for (int i = 0; i < *c_num; i++)
    {
        cout << co_te_ch[i][0] << " \t " << co_te_ch[i][1] << " \t " << co_te_ch[i][2];
        cout << endl;
    }
    system("pause");
    system("CLS");
...
void designTable(string co_te_ch[][3], string table[5][6], int *c_num)
{
